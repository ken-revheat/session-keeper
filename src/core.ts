...
export function computeRefreshDelayMs(
  expiresAtMs: number,
  nowMs: number,
  opts?: { skewMs?: number; minDelayMs?: number; maxDelayMs?: number },
): number {
...
export function readSessionExpiryMs(
  cookieString: string,
  cookieName = "revheat_session_info",
): number | null {
  ...
  const exp = (parsed as { expiresAt?: unknown }).expiresAt;
...
export function safeNextUrl(host: string | null, pathAndQuery: string): string {
 ...
 */
export function shouldBounceToNext(
  status: number,
  next: string | null,
): { bounce: boolean; url?: string } {
  if (status !== 200 || !next) return { bounce: false };
  let u: URL;
  try {
    u = new URL(next);
  } catch {
  ...
  if (u.protocol !== "https:") return { bounce: false };
  const host = u.hostname;
  if (host !== "revheat.com" && !host.endsWith(".revheat.com")) return { bounce: false };
  return { bounce: true, url: safeNextUrl(host, u.pathname + u.search) };
}

export interface SessionKeeperDeps {
  refreshUrl: string; // e.g. https://api.revheat.com/api/auth/refresh
  readExpiryMs: () => number | null; // adapter reads document.cookie
  now?: () => number; // default Date.now
  fetchImpl?: typeof fetch; // default global fetch
  onDead?: () => void; // called once on 401 (adapter may redirect)
}
...
export function createSessionKeeper(deps: SessionKeeperDeps): SessionKeeper {
  ...
  const onFocus = () => {
    ...
    if (expiresAt === null) return;
    if (expiresAt - now() < REFRESH_SKEW_MS) refresh();
    else schedule();
  };
